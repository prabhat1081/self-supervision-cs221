...
from baselines import deepq_self_supervised
from baselines.deepq_self_supervised.replay_buffer import ReplayBuffer, PrioritizedReplayBuffer
from baselines.deepq_self_supervised.utils import ObservationInput

from baselines.common.tf_util import get_session
from baselines.deepq_self_supervised.models import build_q_func
...
class ActWrapper(object):
    ...
    def load_act(path):
        ...
        act = deepq_self_supervised.build_act(**act_params)
        sess = tf.Session()
        sess.__enter__()
...
def learn(env,
    ...
    **network_kwargs
    ...
    assert train_deepq or train_supervised_task
    print("Training deepq: ", train_deepq, "Training self supervised: ", train_supervised_task)
    ...
    act, train, update_target, debug, supervised_task_train, get_grads = deepq_self_supervised.build_train(
        make_obs_ph=make_obs_ph,
        q_func=q_func,
        num_actions=env.action_space.n,
        optimizer=tf.train.AdamOptimizer(learning_rate=lr),
        supervised_task_optimizer=tf.train.AdamOptimizer(learning_rate=lr),
        gamma=gamma,
        ...
        for t in range(total_timesteps):
            ...
            if t > learning_starts and t % train_freq == 0:
                ...
                if(train_supervised_task):
                    #print(obses_t.shape)
                    obses_t_reshaped = np.array(np.transpose(obses_t, (0, 1, 4, 2, 3)), copy = False)
                    ...
                    for i in range(len(obses_t_reshaped)) :
                        ...
                        obses_t_reshaped[i][0] = obses_t_reshaped[i][0][indices[idx]]
                    obses_t_self = np.transpose(obses_t_reshaped, (0, 1, 3, 4, 2))
            ...
            if done and print_freq is not None and len(episode_rewards) % print_freq == 0:
                ...
                logger.record_tabular("supervised task mean error ", np.mean(supervised_task_errors))
