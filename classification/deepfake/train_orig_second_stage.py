...
def save_checkpoint(epoch, model, classifier, checkpoint_dir) :
	outfile = os.path.join(checkpoint_dir, 'backbone.tar')
	torch.save({'epoch':epoch, 'state':model.state_dict()}, outfile)
		
	outfile1 = os.path.join(checkpoint_dir, 'classifier.tar')
...
optimizer = torch.optim.Adam([{'params':classifier.parameters()},{'params':model.parameters()}], lr = 2.5e-4)
