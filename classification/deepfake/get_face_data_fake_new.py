...
fake_train_dir = "fake_train_images"
fake_test_dir = "fake_test_images"
		...
		if frame_idx%sample_idx == 0 :
			...
			if len(faces):
				...
				if not actor_idx in [25, 26, 27, 28] :
					save_image(cropped_face, join(fake_train_dir, str(fake_count)+".png"))
				else :
					save_image(cropped_face, join(fake_test_dir, str(fake_count)+".png"))
				fake_count += 1
