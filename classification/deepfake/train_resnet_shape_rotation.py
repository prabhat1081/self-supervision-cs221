...
from torch.autograd import Variable
from load_model_Resnet_shape import load_model
import sys
...
base_datamgr    = SimpleDataManager(image_size, batch_size = 8)
...
def save_checkpoint(epoch, model, checkpoint_dir) :
	outfile = os.path.join(checkpoint_dir, 'best_model_rotation1.tar')
...
rotate_classifier = nn.Sequential(nn.Linear(2048,4))
rotate_classifier = rotate_classifier.cuda()


optimizer = torch.optim.Adam([{'params':classifier.parameters()},{'params':rotate_classifier.parameters()}], lr = 5e-4)
